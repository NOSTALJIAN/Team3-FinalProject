	...
	<div style="display: flex;">
		...
		<ul class="pagination justify-content-center mt-4">
			<c:if test="${currentBoardPage gt 10}">
				<li class="page-item"><a class="page-link" href="/group/groupMade?p=${startPage - 1}&f=${field}&q=${query}">&laquo;</a></li>
			</c:if>
			...
			<c:forEach var="page" items="${pageList}" varStatus="loop">    
				<li class="page-item ${(currentBoardPage eq page) ? 'active' : ''}">
					<a class="page-link" href="/group/groupMade?p=${page}&f=${field}&q=${query}">${page}</a>
				</li>
			</c:forEach>  
			<c:if test="${totalPages gt endPage}">                    
				<li class="page-item"><a class="page-link" href="/group/groupMade?p=${endPage + 1}&f=${field}&q=${query}">&raquo;</a></li>
			</c:if>
